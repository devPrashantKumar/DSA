...
public class SurroundedRegions {
    ...
    * Space Complexity : O(m*n)
    ...
    public static void boundaryBFS(char[][] board) {
        boolean[][] visited = new boolean[board.length][board[0].length];
        Queue<Pair> queue = new LinkedList<>();
        int[][] deltaRowCol = {{-1,0,1,0},{0,-1,0,1}};
        ...
        for(int i=0;i<board.length;i++){
            for(int j=0;j<board[i].length;j++){
                if(!visited[i][j] && board[i][j]=='O' && (i==0 || j==0 || i==board.length-1 || j==board[i].length-1)){
                    queue.offer(new Pair(i,j));
                    visited[i][j]=true;
                    while(!queue.isEmpty()){
                        Pair it = queue.poll();
    ...
    public static void normalBFS(char[][] board) {
        boolean[][] visited = new boolean[board.length][board[0].length];
        Queue<Pair> queue = new LinkedList<>();
        int[][] deltaRowCol = {{-1,0,1,0},{0,-1,0,1}};
    ...
    public static void main(String[] args) {
        char[][] grid10 = {{'X','X','X','X'},{'X','O','O','X'},{'X','X','O','X'},{'X','O','X','X'}};
        solve(grid10);
        System.out.println("grid1 : "+Arrays.deepToString(grid10));

        char[][] grid11 = {{'X','X','X','X'},{'X','O','O','X'},{'X','X','O','X'},{'X','O','X','X'}};
        solveApproach2(grid11);
        System.out.println("grid1 : "+Arrays.deepToString(grid11));
        ...
        char[][] grid21 = {{'X'}};
        solveApproach2(grid21);
        System.out.println("grid2 : "+Arrays.deepToString(grid21));
        System.out.println("-------------------------------------------------------------");
    }
}
